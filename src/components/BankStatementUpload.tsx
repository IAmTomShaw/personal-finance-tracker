...
export const BankStatementUpload: React.FC = () => {
  ...
  const handleImport = () => {
    ...
    try {
      ...
      setStatusMessage(
        `Imported ${importEntries.length} daily balance${importEntries.length === 1 ? '' : 's'} for ${
  ...
  if (accounts.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-gray-500 text-lg mb-4">No accounts available</div>
        <p className="text-gray-400">Create an account first, then upload your bank statement CSV.</p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6 space-y-8">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Bank Statement Import</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label htmlFor="statement-type" className="block text-sm font-medium text-gray-700 mb-2">
              Statement Type
            </label>
            <select
              id="statement-type"
          ...
          <div>
            <label htmlFor="statement-upload" className="block text-sm font-medium text-gray-700 mb-2">
              CSV Bank Statement
            </label>
            <input
              id="statement-upload"
              ...
              className="w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700"
            />
            {fileName && (
              <p className="text-xs text-gray-500 mt-2">Selected file: {fileName}</p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="rounded-lg border bg-gray-50 px-4 py-3">
                ...
                <div className="text-lg font-semibold text-gray-800">{summary.totalRows}</div>
              </div>
              <div className="rounded-lg border bg-gray-50 px-4 py-3">
                <div className="text-xs uppercase text-gray-500">
                  {statementType === 'transactions' ? 'Valid Txns' : 'Valid Rows'}
                </div>
                <div className="text-lg font-semibold text-gray-800">{summary.validRows}</div>
              </div>
              <div className="rounded-lg border bg-gray-50 px-4 py-3">
                <div className="text-xs uppercase text-gray-500">Unique Days</div>
                <div className="text-lg font-semibold text-gray-800">{summary.uniqueDays}</div>
              </div>
              <div className="rounded-lg border bg-gray-50 px-4 py-3">
                <div className="text-xs uppercase text-gray-500">Skipped</div>
                <div className="text-lg font-semibold text-gray-800">{summary.skippedRows}</div>
              </div>
            </div>
              ...
              {summary.duplicateCount > 0 && (
                <span className="ml-2 text-amber-600">
                  {statementType === 'balance'
                    ? `${summary.duplicateCount} duplicate date${
                        summary.duplicateCount === 1 ? '' : 's'
                      } found. The last row per day will be used.`
                    : `${summary.duplicateCount} additional transaction${
                        summary.duplicateCount === 1 ? '' : 's'
